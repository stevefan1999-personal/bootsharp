// <auto-generated from C# WorkerEntrypoint / DurableObject / WorkflowEntrypoint />
...
import wasmModule from "../../../dist/wasm/backend.wasm";
...
const wrappedEnvs = new WeakMap();

export function wrapEnv(env) {
  if (!env) missing("env");
  const cached = wrappedEnvs.get(env);
  if (cached) return cached;
  const wrapped = {
    KV: wrapIKvNamespace(env.KV),
    DB: wrapID1Database(env.DB),
    BUCKET: wrapIR2Bucket(env.BUCKET),
    QUEUE: wrapIQueue(env.QUEUE),
    COUNTER: wrapICounterNamespace(env.COUNTER),
    WORKFLOW: wrapIWorkflow(env.WORKFLOW),
    ENVIRONMENT: wrapIdentity(env.ENVIRONMENT),
  };
...
function wrapIdentity(value) { return value; }
async function rpcNumber(pending, name) {
  const value = Number(await pending);
  if (!Number.isFinite(value)) throw new Error(`${name} did not return a number`);
  return { value };
}
function wrapICounterNamespace(ns) {
  if (!ns) missing("ICounterNamespace");
  return {
    newUniqueId: () => wrapDoId(ns.newUniqueId()),
    idFromName: (name) => wrapDoId(ns.idFromName(name)),
    idFromString: (id) => wrapDoId(ns.idFromString(id)),
    get: (id) => wrapICounterStub(ns.get(ns.idFromString(id))),
    getByName: (name) => wrapICounterStub(ns.getByName(name)),
  };
}
function wrapICounterStub(stub) {
  const wrap = Object.create(null);
  wrap.id = String(stub.id);
  wrap.name = stub.name ?? null;
  wrap.get = (...args) => rpcNumber(stub["get"](...args), "get");
  wrap.increment = (...args) => rpcNumber(stub["increment"](...args), "increment");
  wrap.sqlDemo = (...args) => stub["sqlDemo"](...args);
  return wrap;
}
...
export class Counter extends DurableObject {
  ...
  async get(...args) {
    try {
      ...
      const json = await reentrant(() => {
        this._id ??= IActorRuntime.constructDurableObject("Counter", wrapState(this.ctx), wrapEnv(this.env));
        return IActorRuntime.callDurableObject(this._id, "get", JSON.stringify(args));
      });
  ...
  async increment(...args) {
    try {
      ...
      const json = await reentrant(() => {
        this._id ??= IActorRuntime.constructDurableObject("Counter", wrapState(this.ctx), wrapEnv(this.env));
        return IActorRuntime.callDurableObject(this._id, "increment", JSON.stringify(args));
      });
      return JSON.parse(json);
    } catch (error) {
  ...
  async sqlDemo(...args) {
    try {
      ...
      const json = await reentrant(() => {
        this._id ??= IActorRuntime.constructDurableObject("Counter", wrapState(this.ctx), wrapEnv(this.env));
        return IActorRuntime.callDurableObject(this._id, "sqlDemo", JSON.stringify(args));
      });
      return JSON.parse(json);
    } catch (error) {
      console.error("Counter.sqlDemo failed", error);
      throw new Error("Internal server error");
    }
  }
}
  ...
  async run(event, step) {
    try {
      ...
      return JSON.parse(await reentrant(() => {
        this._id ??= IActorRuntime.constructWorkflow("DemoWorkflow", this.ctx, wrapEnv(this.env));
        return IActorRuntime.runWorkflow(this._id, payload, wrapStep(step));
      }));
    } catch (error) {
      console.error("DemoWorkflow.run failed", error);
      throw new Error("Internal server error");
    }
  }
}
export default class Worker extends WorkerEntrypoint {
  ...
  async queue(batch) {
    try {
      ...
      console.error("Worker.queue failed", error);
      batch.retryAll();
    }
  }
  async scheduled(controller) {
    try {
      ...
      console.error("Worker.scheduled failed", error);
      throw new Error("Internal server error");
    }
  }
}
