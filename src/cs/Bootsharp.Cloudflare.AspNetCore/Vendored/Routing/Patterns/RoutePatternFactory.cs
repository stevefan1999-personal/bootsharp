...
// upstream: src/Http/Routing/src/Patterns/RoutePatternFactory.cs
...
#endif
{
    ...
    public static RoutePatternParameterPart ParameterPart(string parameterName)
    {
        ...
        if (parameterName.ContainsAny(RoutePatternParser.InvalidParameterNameChars))
        {
...
#if !BSCF_WORKERS // excised: the string overload compiles the token into a RegexRouteConstraint, which is the one constraint that roots System.Text.RegularExpressions; the remaining else throws for an unrecognised constraint object
